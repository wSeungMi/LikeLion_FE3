...
function StudyDetail(props) {
    // 이전 버전에서는 props로 처리했음
    // console.log(props) // props로 받으면 들어온 값이 없음
    ...
    //fetch 예시
    //fetch(`url`/${num}) 이런식으로 데이터를 가지고 와서 아래에다가 데이터를 뿌려주면 블로그 형식이 완성되는 것!

    return (
        <div>
            {num}번 게시물에 오신 것을 환영합니다. 여기서 fetch를 통해
            백엔드로부터 {num}번 게시물을 요청하셔야 합니다.
        </div>
    );
}

// route를 /study/:num/:test라고 했을 때
// function Test(){
//   const location = useLocation()
...
function Chat() {
    ...
    // 호준님이 자주 하시는 방식(권하진 않으심)
    // 왜 이렇게 사용하냐면 url의 주소가 깊어졌을 때 www/어쩌구.com/a/b/c/d/e 이렇게 되었을 때 앞에 있는 값이 필요한 경우가 종종 있어서 코드의 일관성을 유지하기 위해 단순한 것도 사용함
    return (
        <p>
            {채팅방번호}번 게시물에 오신것을 환영합니다. 여기는 {채팅방번호}번
            게시물의 메모방입니다. 여러분이 fetch를 통해 백엔드에서 채팅 정보를
            받아와야 합니다.
        </p>
    );
}

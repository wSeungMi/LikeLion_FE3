...
// 초기 화면에서 저장되어 있는 데이터를 가지고 목록을 생성한다.
if (tasks.length > 0) {
    tasks.forEach((task) => {
...
// 할일 정보를 업데이트하는 함수 (할일을 클릭했을 때 혹은 할일을 제거했을 때 동작함)
function buildTasks() {
    tasks.length = 0; // 업데이트 하기 전에 기존 데이터를 초기화한다.

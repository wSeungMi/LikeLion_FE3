...
// replace는 제일 먼저 일치하는 패턴만 변환
('likelionFE3 Student wooseungmi hello Student').replace(/Student/, "Developer")

// global하게 해당되는 문자를 전부 변경
('likelionFE3 Student wooseungmi hello Student').replace(/Student/g, "Developer")

('likelionFE3 Student wooseungmi hello Student').match(/Student/g) //['Student', 'Student']

('likelionFE3 Student wooseungmi hello Student').split(/Student/g)

(/Student/g).test('likelionFE3 Student wooseungmi hello Student')
...
// 6. 모든 문자 매핑
/h[eao]llo/g // []안의 글자만 해당되는 걸 찾아줌

// 7. 범위
/h[a-zA-Z0-9]llo/g // []안의 범위 중에 하나만 찾음

// 8. 부정
/h[^ea]llo/g // []안의 글자가 아닌 것만 찾음
...
/.(a|e|o)ll./gm
/hello (?!world)/gm // hello 뒤에 world가 오지 않는 것(nagative 매칭)
/hello (?=world)/gm // hello 뒤에 world가 오는 것(positive 매칭)
...
/[0-9a-zA-Z]{2,3}[-.* ]?[0-9]{3,5}[-.* ]?[0-9]{4}/gm

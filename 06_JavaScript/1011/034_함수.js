...
// 1. 재사용성 -> 재사용하지 않을 코드는 함수로 정의하나요? -> 가능하면 함수로 만드는 것을 권장함
...
// 콘솔창의 기능. 마지막 라인에 한하여 console.log()를 찍지 않아도 return값을 console창에 출력해줌
함수1(10, 20, 30) 
함수1(10, 20, 50)
...
// 필요 이상의 아규먼트를 넣었을 때
함수1(10, 20, 30, 40) // Error를 뿜지 않는다. 60
...
// 3. 기본값 할당
function 함수3(a=10, b=10, c=10) {
    return a + b + c
}

console.log(함수3()) // 30
console.log(함수3(100)) // 120
console.log(함수3(100, 200)) // 310
console.log(함수3(100, 200, 300)) // 600

// 값이 순서대로 들어감
console.log(함수3(c=1000)) // 1030이 되어야하는데 왜 1050이 되었을까?

console.log(함수3(c=1000, a=2000)) // 3020이 되어야하는데 왜 3030이 되었을까?

// 4. 함수에서 객체를 활용해서 아규먼트로 (중요)
...
// 시그마 = 누적합
// 누적합 아래 코드들이 좋은 코드는 아님(수학공식으로 푸는게 더 효율적)
function 누적합(n) {
    if (n <= 1){ // 종료조건이 없으면 무한반복됨
        return 1
    }
...
// 수학공식 사용버전
// 순회를 돌지 않고 해결할 수 있으면 돌지 않고 해결하는게 좋음 -> 더 좋은 코드
let n = 100
console.log(n * (n + 1) / 2)
...
// 지역변수와 전역변수
// 블록레벨스코프
if (true) { // for문이어도 마찬가지
    let x = 10;
    const y = 10;
}
console.log(x, y); // if문 안의 값을 호출 X

// 함수 밖에 선언된 x는 함수 내부에서도 접근이 가능함
let x = 100
function xplus() {
    x = x + 10
}
...
// 함수 안에 함수
// 함수가 종료된 후, 선언된 함수도 변수와 마찬가지로 휘발됨
function a() {
    console.log('a 실행')
    function b() { 
        console.log('b 실행')

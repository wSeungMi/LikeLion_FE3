...
3. 공백 병합
    1. 함수나 메서드 내 공백 병합
    2. obejct형 자료형 내 공백 병합  
    3. 선언식에서의 공백 병합  
    4. 메서드 체이닝에서의 공백 병합
    5. 주의 : ''(작은 따옴표), ""(큰 따옴표)는 개행 공백을 인식하지 못함 -> `` (백틱/템플릿 리터럴)은 개행 공백을 인식함  
...
4. 주석
* 주석은 겹칠 수 없음
* 실무에서는 주석을 굳이 읽지 않아도 코드만 보고 해석되는 코드를 더 선호함  
```js
// 한 줄 주석입니다.
/*
여러줄 주석입니다.
...
-   var, let, const
    ...
    -   const(ES5) : 블록 레벨 스코프, 재선언시 에러 O, 콘솔에서는 에러 X, 변경이 불가능한 자료형(상수)  
        - 모든 변수는 const를 사용하고, let을 꼭 사용해야 하는 변수에서만 let을 사용하길 권고함!
            ```js
            function hello() {
                const x = 10
                /// 1000줄의 코드
            ...
            ```
        * DOM을 탐색해야 하는 경우 const를 사용하고, (간혹) $를 하는 경우도 있음. 반복문이나 함수 밖에서 주로 사용함
            ```js
...
-   산술 연산자(+, -, /, \*, \*\*, %)
-   할당 연산자(=), 복합 할당 연산자(+=, -=, /=, \*=, \*\*=, %=)
-   논리 연산자(&&, ||, !, !!, &, |, ~) <!-- ~는 비트 연산자로 Math.floor랑 같음. 단, 닌자코드가 될 확률이 높아서 협업시 사용 권장 X --> 
    ...
    -   ||는 합
    -   !는 부정
    -   !!는 true와 false를 반환하게 해주는 코드 (부정의 부정)
    -   암기코드
...
-   비교 연산자(>, >=, <, <=, ==, !=, ===, !==)
    - 문자열 비교 : 유니코드 순
        - 문자열 순서대로 하나씩 비교하고, 문자가 모두 같은데 길이가 다른 경우 길이가 더 긴 것이 true
            ```js
            e.g. wooseungmi > wooseungmii // false
            ```
    - 다른 형을 가진 값 간의 비교
    ...
    - 에지 케이스(edge case)
        ```js
        console.log(null == 0) // false (null이 0으로 변환)
        console.log(null === 0) // false (null이 0으로 변환되지 않음)
        console.log(null >= 0) // true (null이 0으로 변환)
        // >= 연산을 할 때 null이 0으로 변환이 되어 위와 같은 일을 만듦
        ```
        ```js
        // 비교가 불가능한 undefined
        // 비교연산자가 피연산자로 NaN을 만나면 뒤도 돌아 보지 않고 false를 반환함
        console.log(undefined > 0) // false (undefined가 NaN으로 변환)
        console.log(undefined < 0) // false (undefined가 NaN으로 변환)
        console.log(undefined == 0) // false (undefined와 null이 같고, 그 외에 값은 false)
        ```

-   단항 산술 연산자(++x, x++, --x, x--)
-   nullish 병합 연산자(??) <!-- null과 undefined만 판단 -->
-   비트 연산자(AND - &, OR - |, XOR - ^, NOT - ~, 왼쪽 시프트 - <<, 오른쪽 시프트 - >>, 부호 없는 오른쪽 시프트 ->>>) 
    ...
    let x = 0;
    console.log(x || 10); // 10 (||는 0을 false로 판단)
...
-   Number(숫자)
    ...
    -   형태 : 10, 10.123, -10, Infinity(e.g. 1 / 0), -Infinity, NaN(e.g."hello"/2)
    -   호출 : 변수명
    -   메서드 :
        ...
        - Number.MAX_VALUE : JS가 표현할 수 있는 제일 큰 양의 숫자  
    -   Number()
    -   parseInt() - 권고, parseFloat()
        ```js
        parseInt("1hello world"); // 1
        Number("1hello world"); // NaN (공백이나 다른 문자열이 혼합됐을 때 형변환을 제대로 못해줌)
        ```
        <img src="img/number&parseInt.png">
    ...
    - BigInt
        - (2**53-1)(9007199254740991) 보다 큰 값을 Number로 나타낼 수 없으며, 나타내도 안전한 연산을 할 수 없음
            ```js
            const bigInt = 123123123123123123123123123123123123123123123123123n;
            ```
...
-  명시적 형변환이 아니라 암시적(묵시적) 형변환
    ...
    ```
- 0, null, undefined, NaN, ""은 false이며 그 외의 값은 true!
- +를 사용해서 2개를 연산할 때에 하나가 문자열이면 나머지 하나도 문자열로 연산함 (뺄셈과 나눗셈, 곱셈의 방식은 다름)
    ```js
    2 + "2" // '22'
    1 + 1 + 3 + '2' + '2' // 522 
...
-   switch
    ```js
    // if문을 한 줄로 사용하는 경우도 종종 있음.
    if (true) console.log('hello world');
    ```
    ```js
    // if문 안에서 선언된 변수(let, const)는 밖에서 읽을 수 없다.
    // let, const는 블록 스코프를 가지고 있기 때문!
    if (true) {
        let x = 10
        console.log(x)
    }
    console.log(x) // if문 안의 변수를 읽을 수 없다.
    ```
    ```js
    if(1) console.log('1')
    if('') console.log('공백없음')
    ...
    - 문제 : 다음 if문을 삼항 연산자로 만들어보세요.
        ...
        // 문제풀이
        let money = 500;
        (money < 1300) ? '버스를 탈 수 없습니다' : '버스를 탔습니다.';
        ```
        ```js
        // 문제 2번
        let money = 500
...
// 위험한 무한반복..! 무한반복은 메모리를 끝까지 다잡아먹어서 잘못하면 서비스를 멈추게 할 수 있음 (== 이렇게 등호만 쓰는 것도 위험함, 연산하다가 부동소수점(무한대수)를 만날 수도 있기때문)
let x = 0;
while (true) {
    console.log(x);
    x += 2;
